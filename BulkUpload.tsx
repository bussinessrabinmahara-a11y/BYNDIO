...
export default function ProductCard({ product }: { product: Product }) {
  ...
  return (
    ...
    >
      {/* Image area */}
      <div className="h-[155px] flex items-center justify-center text-[52px] bg-gray-50 relative overflow-hidden">
        {disc > 0 && (
          <div className="absolute top-2 left-2 bg-[#388E3C] text-white text-[10px] font-bold px-1.5 py-0.5 rounded-sm">
            {disc}% off
          </div>
        )}
        <button
          onClick={handleWishlist}
          className="absolute top-2 right-2 bg-white/90 hover:bg-white border-none rounded-full w-7 h-7 flex items-center justify-center cursor-pointer transition-transform hover:scale-110 shadow-sm"
        ...
        {product.inf && (
          <div className="absolute bottom-2 left-2 bg-[#7B1FA2] text-white text-[9px] font-bold px-1.5 py-0.5 rounded-full">
            ⭐ Creator Pick
          </div>
        )}
