...
export default function RewardsWallet() {
  ...
  const fetchHistory = async () => {
    ...
    try {
      ...
      console.error('fetchHistory:', err?.message || err);
    }
  };

  const handleRedeem = async () => {
    if (rewardPoints < 100) { toast('Minimum 100 points needed to redeem.', 'info'); return; }
    toastSuccess(`Redeemed ${rewardPoints} points = ₹${(rewardPoints * 0.1).toFixed(0)} added to wallet!`);
  };
      ...
      <div className="max-w-4xl mx-auto p-4 md:p-6 flex flex-col gap-4">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          ...
          <div className="bg-gradient-to-br from-[#0D47A1] to-[#1565C0] text-white rounded-xl p-5">
            ...
            <button className="bg-white text-[#0D47A1] px-4 py-2 rounded-md text-[12px] font-bold hover:bg-blue-50 transition-colors">
              Withdraw to Bank
            </button>
          </div>
        </div>
