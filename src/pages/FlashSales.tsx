...
export default function FlashSales() {
  ...
  const flashProducts = flashSales.length > 0 ? flashSales.map(fs => products.find(p => p.id.toString() === fs.product_id)).filter(Boolean) : products.slice(0, 8).map(p => ({ ...p, flash_sale: { discount_pct: Math.floor(Math.random() * 30) + 20, ends_at: new Date(Date.now() + 3600000 * 3).toISOString(), sale_price: Math.round(p.price * 0.7) } }));
