...
function PromoCodeSection({ userId, affiliateLinks }: { userId?: string; affiliateLinks: any[] }) {
  ...
  const handleCreate = async () => {
    ...
    const newPromo = { ...form, id: Date.now(), uses: 0, status: 'active', created_at: new Date().toISOString() };
            ...
            <div key={p.id} className="flex items-center gap-3 p-4 border-b border-gray-50 last:border-0">
              <div className="flex-1">
                ...
                <div className="text-[11px] text-gray-500 mt-0.5">
                  {p.discount}{p.type === 'percentage' ? '%' : '₹'} off • {p.product === 'all' ? 'All products' : 'Specific product'} • {p.uses} uses
...
function PerformanceRanking({ totalClicks, totalConversions, totalEarnings, convRate, userId }: any) {
  const MOCK_LEADERBOARD = [
    { rank: 1, name: 'Priya Sharma', earnings: 48200, conversions: 234, badge: '👑' },
    { rank: 2, name: 'Rahul Verma', earnings: 38900, conversions: 189, badge: '🥈' },
    { rank: 3, name: 'Anjali Singh', earnings: 31500, conversions: 156, badge: '🥉' },
    { rank: 4, name: 'Vikram Patel', earnings: 24300, conversions: 121, badge: '⭐' },
    { rank: 5, name: 'Sneha Gupta', earnings: 19800, conversions: 98, badge: '⭐' },
    { rank: 6, name: 'You', earnings: totalEarnings, conversions: totalConversions, badge: '📍', isYou: true },
  ].sort((a, b) => b.earnings - a.earnings).map((r, i) => ({ ...r, rank: i + 1 }));

  const myRank = MOCK_LEADERBOARD.find(r => r.isYou)?.rank || 0;
  const myPercentile = Math.round((1 - (myRank / MOCK_LEADERBOARD.length)) * 100);

  const tiers = [
    { name: 'Bronze', min: 0, max: 5000, icon: '🥉', color: '#CD7F32' },
    { name: 'Silver', min: 5000, max: 20000, icon: '🥈', color: '#9E9E9E' },
    { name: 'Gold', min: 20000, max: 50000, icon: '🥇', color: '#F9A825' },
    { name: 'Platinum', min: 50000, max: Infinity, icon: '💎', color: '#1565C0' },
  ];
              ...
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                {[
                  ...
                  <div key={i} className={`border-2 ${plan.color} rounded-xl p-4 relative`}>
                    ...
                    <button className="w-full mt-3 bg-[#7B1FA2] hover:bg-[#6A1B9A] text-white py-2 rounded-md text-[12px] font-bold transition-colors">
                      Subscribe
                    </button>
                  </div>
                ))}
