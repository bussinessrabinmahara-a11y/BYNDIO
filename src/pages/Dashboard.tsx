                    ...
                    {orders.map((o, i) => (
                      <tr key={i} className="hover:bg-blue-50/50">
                        ...
                        <td className="p-3 border-b border-gray-200">
                          <button onClick={() => toastSuccess('Tracking details sent to buyer')} className="text-[11px] text-[#1565C0] font-bold hover:underline mr-2">Track</button>
                    ...
                    {products.map((p, i) => (
                      <tr key={i} className="hover:bg-blue-50/50">
                        ...
                        <td className="p-3 border-b border-gray-200 flex gap-2 items-center">
                          <button onClick={() => toast('Product editing coming soon!', 'info')} className="text-[11px] text-[#1565C0] font-bold hover:underline">Edit</button>
                          <button onClick={async () => {
            ...
            <div className="bg-white rounded-[10px] p-5 shadow-sm mb-4">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                <div className="border border-gray-200 rounded-lg p-4">
                  ...
                  <div className="text-[11px] text-gray-500 mb-3">7-day settlement to bank account. No fee.</div>
                  <button onClick={() => toastSuccess('Withdrawal request submitted! Funds arrive in 7 working days.')} className="bg-[#388E3C] hover:bg-[#2E7D32] text-white px-4 py-2 rounded-md text-[12px] font-bold transition-colors">💸 Withdraw Funds</button>
                </div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                {[
                  ...
                  <div key={i} className={`border-2 ${plan.color} rounded-xl p-4 relative`}>
                    ...
                    <button className="w-full mt-3 bg-[#0D47A1] hover:bg-[#1565C0] text-white py-2 rounded-md text-[12px] font-bold transition-colors">
              ...
              {products.length === 0 ? (
                ...
                <div className="flex flex-col gap-2">
                  {products.slice(0, 5).map((p, i) => {
                    const rev = orders.filter(o => o.products?.name === p.name).reduce((s, o) => s + o.price * o.quantity, 0);
                    const pct = products.length ? Math.min(100, Math.round((i === 0 ? 80 : 80 - i * 15))) : 0;
                    return (
                      <div key={p.id} className="flex items-center gap-3">
                        ...
                        <div className="flex-1 min-w-0">
                          ...
                          <div className="h-1.5 bg-gray-100 rounded-full mt-1 overflow-hidden">
                            <div className="h-full bg-[#0D47A1] rounded-full" style={{ width: `${pct}%` }} />
                          </div>
                        </div>
                        <div className="text-[12px] font-black text-[#388E3C] shrink-0">₹{rev > 0 ? rev.toLocaleString('en-IN') : p.price.toLocaleString('en-IN')}</div>
                      </div>
                    );
                  })}
                </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
              <div className="bg-white rounded-[10px] p-5 shadow-sm">
                ...
                <div className="flex flex-col gap-3">
                  {[
                    { label: 'Store Name', placeholder: `${user?.name}'s Store`, type: 'text' },
                    { label: 'Business Email', placeholder: user?.email || 'store@example.com', type: 'email' },
                    { label: 'Support Phone', placeholder: '+91 98765 43210', type: 'tel' },
                    { label: 'Store Location', placeholder: 'Mumbai, Maharashtra', type: 'text' },
                  ].map(f => (
                  ...
                  <button onClick={() => toastSuccess('Settings saved successfully!')}
                    className="bg-[#0D47A1] hover:bg-[#1565C0] text-white py-2.5 rounded-md text-[13px] font-bold transition-colors mt-1">
                    Save Changes
                  </button>
                </div>
              </div>
              <div className="bg-white rounded-[10px] p-5 shadow-sm">
                ...
                <div className="flex flex-col gap-3">
                  {[
                    ...
                    { label: 'Bank Name', placeholder: 'HDFC Bank', type: 'text' },
                  ].map(f => (
                    <div key={f.label} className="flex flex-col gap-1">
                      <label className="text-[11px] font-bold text-gray-500 uppercase tracking-wide">{f.label}</label>
                      <input type={f.type} placeholder={f.placeholder}
                  ...
                  <button onClick={() => toastSuccess('Bank details saved!')}
                    className="bg-[#388E3C] hover:bg-[#2E7D32] text-white py-2.5 rounded-md text-[13px] font-bold transition-colors mt-1">
                    💰 Save Bank Details
                  </button>
                </div>
              </div>
              <div className="bg-white rounded-[10px] p-5 shadow-sm">
                ...
                <div className="flex flex-col gap-3">
                  ...
                  <button onClick={() => toastSuccess('KYC documents submitted for verification!')}
                    className="bg-[#7B1FA2] hover:bg-[#6A1B9A] text-white py-2.5 rounded-md text-[13px] font-bold transition-colors">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-3.5">
              <div className="bg-white border border-gray-200 rounded-[10px] p-5 flex items-start gap-3.5">
                ...
                <div>
                  ...
                  <div className="text-xs text-gray-500 leading-relaxed mb-2">Get featured on homepage for 10X more views. ₹2,999/week</div>
                  <button onClick={() => toastSuccess('Homepage boost activated for 7 days!')} className="bg-[#E65100] hover:bg-[#F57C00] text-white px-4 py-2 rounded text-xs font-bold transition-colors">Boost Now</button>
                </div>
              </div>
              <div className="bg-white border border-gray-200 rounded-[10px] p-5 flex items-start gap-3.5">
                <div className="text-[28px] shrink-0">📊</div>
                <div>
                  ...
                  <div className="text-xs text-gray-500 leading-relaxed mb-2">Appear first in category search results. ₹999/week</div>
                  <button onClick={() => toastSuccess('Sponsored listing activated successfully!')} className="bg-[#E65100] hover:bg-[#F57C00] text-white px-4 py-2 rounded text-xs font-bold transition-colors">Sponsor Now</button>
                </div>
              </div>
            </div>
          </>
